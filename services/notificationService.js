...
class NotificationService {
  ...
  async checkAndNotifyHarvestDue() {
    try {
      ...
      console.log({ from: startOfDay, to: endOfDay });
      ...
      // Manual filtering because whenToPluck is stored as string
      const cropsDue = crops.filter(crop => {
        const pluckDate = new Date(crop.whenToPluck);
        return pluckDate >= startOfDay && pluckDate < endOfDay;
      });

      console.log('Filtered crops due today:', cropsDue.map(c => c.name));
      logger.info(`Found ${cropsDue.length} crops due for harvest`);

      for (const crop of cropsDue) {

        if (!crop.userId.notificationPreferences?.harvestReminders) continue;

        const pluckDate = new Date(crop.whenToPluck);
        const isToday = pluckDate.toDateString() === today.toDateString();
        const isTomorrow = pluckDate.toDateString() === new Date(today.getTime() + 86400000).toDateString();
